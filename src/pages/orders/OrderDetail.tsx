...
export default function OrderDetail() {
  ...
  <Text style={styles.smallAddress}>{order.address}</Text>
      ...
      <View style={styles.totalsCard}>
        <View style={styles.totalsRow}>
          ...
          <Text style={styles.totalsValue}>₹{(order.totalAmount * (order.taxes/100)).toFixed(2)}</Text>
        </View>
        <View style={styles.totalsRow}>
  ...
  <Text style={styles.sectionTitleMargin6}>Status</Text>
      <View style={styles.statusRow}>
        <TouchableOpacity
          style={[styles.statusBtn, order.status.received && styles.statusActive]}
          onPress={() => toggle('received')}
        >
          <Text style={styles.statusText}>✅ Received</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.statusBtn, order.status.couriered && styles.statusActive]}
          onPress={() => toggle('couriered')}
        >
          <Text style={styles.statusText}>📦 Couriered</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.statusBtn, order.status.delivered && styles.statusActive]}
...
const styles = StyleSheet.create({
  ...
  small: { color: '#7a6258' },
  card: { backgroundColor: '#fff', borderRadius: 12, padding: 12, marginBottom: 12, borderWidth: 1, borderColor: '#f0e7e3' },
  cardName: { fontSize: 16, fontWeight: '700', color: '#3a241f' },
  sectionTitle: { fontSize: 16, fontWeight: '700', color: '#3a241f', marginBottom: 8 },
  itemsContainer: { marginBottom: 8 },
  itemCard: { backgroundColor: '#fff', borderRadius: 10, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: '#f0e7e3', flexDirection: 'row', justifyContent: 'space-between' },
  itemName: { color: '#3a241f', fontWeight: '600' },
  itemPrice: { color: '#3a241f', fontWeight: '700' },
  totalsCard: { backgroundColor: '#fff6f4', padding: 12, borderRadius: 10, marginVertical: 8, borderWidth: 1, borderColor: '#eee' },
  ...
  statusBtn: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, marginRight: 8, marginBottom: 8, backgroundColor: '#eadfd9' },
  statusActive: { backgroundColor: '#6e4337' },
  statusText: { color: '#5b4037', fontWeight: '700' },
  timelineContainer: { marginTop: 8, marginBottom: 20 },
  timelineRow: { backgroundColor: '#fff', borderRadius: 10, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: '#f0e7e3', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  totalsRowMarginTop: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4, marginTop: 8 },
  sectionTitleMargin6: { fontSize: 16, fontWeight: '700', color: '#3a241f', marginBottom: 8, marginTop: 6 },
  sectionTitleMargin12: { fontSize: 16, fontWeight: '700', color: '#3a241f', marginBottom: 8, marginTop: 12 },
});
