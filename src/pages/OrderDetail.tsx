...
import { View, Text, Button, StyleSheet, FlatList, TouchableOpacity, Share, ScrollView } from 'react-native';
...
export default function OrderDetail() {
  ...
  const order = orders.find((o: Order) => o.id === orderId) as Order | undefined;
  ...
  async function onShareInvoice() {
    ...
    lines.push('');
    order.items.forEach(it => lines.push(`${it.name} x${it.quantity} @${it.rate} = ${it.total}`));
    lines.push('');
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 12 }}>
      ...
      <FlatList data={order.items} keyExtractor={it => it.id} renderItem={({ item }) => (
        <View style={styles.itemRow}><Text>{item.name}</Text><Text>₹{item.total.toFixed(2)}</Text></View>
      )} />
      ...
      <View style={{ marginTop: 16 }}>
        ...
        <View style={{ flexDirection: 'row', marginTop: 8 }}>
          <TouchableOpacity style={[styles.statusBtn, order.status.received && styles.statusActive]} onPress={() => toggle('received')}><Text>✅ Received</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.statusBtn, order.status.couriered && styles.statusActive]} onPress={() => toggle('couriered')}><Text>📦 Couriered</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.statusBtn, order.status.delivered && styles.statusActive]} onPress={() => toggle('delivered')}><Text>🚚 Delivered</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.statusBtn, order.status.paid && styles.statusActive]} onPress={() => toggle('paid')}><Text>💰 Paid</Text></TouchableOpacity>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  statusBtn: { padding: 8, borderRadius: 6, marginRight: 8, borderWidth: 1, borderColor: '#ddd' },
